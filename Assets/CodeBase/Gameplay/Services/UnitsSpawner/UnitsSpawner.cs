...
namespace CodeBase.Gameplay.Services.UnitsSpawner
{
    public class UnitsSpawner : IUnitsSpawner
    {
        ...
        public async UniTask<Unit> Spawn(Vector2Int coordinates, UnitType unitType, int unitsAmount, TeamID teamID)
        {
            ...
            {
                _logger.LogError($"Unable to spawn {nameof(UnitLogic)}: {unitType} at: {coordinates}. {nameof(TileLogic)} not walkable");
                return null;
            }
            
            if (tile.Logic.IsOccupied == true)
            {
                _logger.LogError($"Unable to spawn {nameof(UnitLogic)}: {unitType} at: {coordinates}. {nameof(TileLogic)} already occupied");
                return null;
            }
